...
import "./@aragon/os/common/IForwarder.sol";

import "./@aragon/os/lib/math/SafeMath.sol";
import "./@aragon/os/lib/math/SafeMath64.sol";
...
contract Voting is IForwarder, AragonApp {
    ...
    event CastVote(uint256 indexed voteId, address indexed voter, bool supports_, uint256 stake);
    ...
    event CastVoteAsDelegate(uint256 indexed voteId, address indexed delegate, address indexed voter, bool supports_, uint256 stake);
    ...
    */
    function newVote(bytes calldata _executionScript, string calldata _metadata) external auth(CREATE_VOTES_ROLE) returns (uint256 voteId) {
    ...
    * @return voteId id for newly created vote
    */
    function newVote(bytes calldata _executionScript, string calldata _metadata, bool _castVote_deprecated, bool _executesIfDecided_deprecated)
        external
    ...
    */
     ...
     */
    function attemptVoteForMultiple(uint256 _voteId, bool _supports, address[] memory _voters) public voteExists(_voteId) {
        Vote storage vote_ = votes[_voteId];
        require(_isValidPhaseToVote(vote_, _supports), ERROR_CAN_NOT_VOTE);
    ...
    */
     ...
     * @return votersList the array of delegated voters
     * @return votingPowerList the array of voting power of delegated voters
     */
    function getDelegatedVoters(address _delegate, uint256 _offset, uint256 _limit) external view returns (address[] memory votersList, uint256[] memory votingPowerList) {
     ...
     * @return votingPowerList the array of voting power of delegated voters
     */
    function getDelegatedVotersAtVote(address _delegate, uint256 _offset, uint256 _limit, uint256 _voteId) external view voteExists(_voteId) returns (address[] memory votersList, uint256[] memory votingPowerList) {
        Vote storage vote_ = votes[_voteId];
    ...
    */
     ...
     * @param _voteId Vote identifier
     */
    function getVotersStateAtVote(uint256 _voteId, address[] calldata _voters) external view voteExists(_voteId) returns (VoterState[] memory voterStatesList) {
    ...
    * @return voteId id for newly created vote
    */
    function _newVote(bytes memory _executionScript, string memory _metadata) internal returns (uint256 voteId) {
    ...
    */
    ...
    function _removeDelegatedAddressFor(address _delegate, address _voter) internal {
        ...
        delegatedVoters[_delegate].addresses.pop();
